...
// First created by YuanSu(yuan_sue@yeah.net/963133358@qq.com) on 2022-Nov-17.
// Hand motion model for HandExo or other devices.
// You can get the joint configuration by using the HandExo device.
...
/// fit different palm sizes
#define THUMB_B_X 32.31
#define THUMB_B_Y -9.4438
#define THUMB_B_Z -35
#define THUMB_LENGTH 51.65

#define INDEX_B_X 78.9067
#define INDEX_B_Y 9.43
#define INDEX_B_Z -25.7446
#define INDEX_LENGTH 67.86

#define MIDDLE_B_X 69.22
#define MIDDLE_B_Y -16.74
#define MIDDLE_B_Z -4.39
#define MIDDLE_LENGTH 87.35
...
//
//--------------------------------------------------------------------------
Eigen::Matrix4f finger1_pose;	//thumb fingertip pose
Eigen::Matrix4f finger2_pose;	//index fingertip pose
Eigen::Matrix4f finger3_pose;	//middle fingertip pose

Eigen::Matrix4f aux_finger1_pose;   //For calculate thumb joints
...
public:
    ...
    double filter(double value, TimeStamp timestamp = UndefinedTime) {
        ...
        lasttime = timestamp;
        // estimate the current variation per second    ÿ\C3\EB\B9\C0\BCƵ\B1ǰ\B5ı仯
...
void sensor_logger(float sensor1, float sensor2, float sensor3, float sensor4, float sensor5, float sensor6,
                   float sensor7, float sensor8, float sensor9, float sensor10, float sensor11, float sensor12,
                   float sensor13, float sensor14, float sensor15, float sensor16, float sensor17, float sensor18) {

    std::ofstream outfile;
    std::string filePath = "sensor_logger.txt";
    outfile.open(filePath, std::ios::app);
    if (!outfile) {
        std::cout << "open sensor_logger.txt filed" << std::endl;
        exit(1);
    }
    outfile << sensor1 << " " << sensor2 << " " << sensor3 << " " << sensor4<< " " << sensor5 << " " << sensor6 << " " <<
            sensor7<< " " << sensor8 << " " << sensor9 << " " << sensor10<< " " << sensor11 <<" " << sensor12<<" "<<
            sensor13 << " " << sensor14 << " " << sensor15 << " " << sensor16 << " " << sensor17 << " " << sensor18 << std::endl;

}

void finger_joint_logger(float para1, float para2, float para3, float para4,
                         float para5, float para6, float para7, float para8,
                         float para9, float para10, float para11, float para12) {

    std::ofstream outfile;
    std::string filePath = "finger_joint_logger.txt";
    outfile.open(filePath, std::ios::app);
    if (!outfile) {
        std::cout << "open finger_joint_logger.txt filed" << std::endl;
        exit(1);
    }
    outfile << para1 << " " << para2 << " " << para3 <<" " <<para4<<" "<<
            para5<< " " <<para6<<" " <<para7<<" "<<para8<<" "<<
            para9<< " " <<para10<<" "<<para11<<" "<<para12   <<std::endl;

}

void thumb_logger(float para1, float para2, float para3) {

    std::ofstream outfile;
    std::string filePath = "thumb_logger.txt";
    outfile.open(filePath, std::ios::app);
    if (!outfile) {
        std::cout << "open thumb_logger.txt filed" << std::endl;
        exit(1);
    }
    outfile << para1 << " " << para2 << " " << para3 << std::endl;
...
void index_logger(float para1, float para2, float para3) {

    std::ofstream outfile;
    std::string filePath = "index_logger.txt";
    outfile.open(filePath, std::ios::app);
    if (!outfile) {
        std::cout << "open index_logger.txt filed" << std::endl;
        exit(1);
    }
    outfile << para1 << " " << para2 << " " << para3 << std::endl;
...
void middle_logger(float para1, float para2, float para3) {

    std::ofstream outfile;
    std::string filePath = "middle_logger.txt";
    outfile.open(filePath, std::ios::app);
    if (!outfile) {
        std::cout << "open middle_logger.txt filed" << std::endl;
        exit(1);
    }
    outfile << para1 << " " << para2 << " " << para3 << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Device: HandExo" << std::endl;
    std::cout << "Hand Movement"<<std::endl;
    std::cout << "Created by YuanSu(yuan_sue@yeah.net) on 2022" << std::endl;
...
*/
    ...
    forth_0->m_material->setBlueLightSky();
    forth_1->m_material->setBlueLightSky();
    forth_2->m_material->setBlueLightSky();
    forth_3->m_material->setBlueLightSky();

    little_0->m_material->setBlueLightSky();
    little_1->m_material->setBlueLightSky();
    little_2->m_material->setBlueLightSky();
    little_3->m_material->setBlueLightSky();
...
void updateHaptics(void)
{
    ...
    while (simulationRunning)
    {
        ...
        */
        ...
        sensor_logger(angle[0], angle[1], angle[2], angle[3], angle[4], angle[5],
        ...
        //------------------------------------------------------------------------------
        std::cout<<filtered_angle0<<","<<filtered_angle1<<","<<filtered_angle2<<","<<filtered_angle3<<","<<filtered_angle4<<","<<filtered_angle5<<std::endl;
  ...
  //      std::cout<<filtered_angle0<<","<<filtered_angle1<<","<<filtered_angle2<<","<<filtered_angle3<<","<<filtered_angle4<<","<<filtered_angle5<<std::endl;
        ...
        index_logger(finger2_pose(0, 3), finger2_pose(2, 3), finger2_pose(1, 3));

        middle_logger(finger3_pose(0, 3), finger3_pose(2, 3), finger3_pose(1, 3));

        thumb_logger(aux_finger1_pose(0, 3), aux_finger1_pose(2, 3), aux_finger1_pose(1, 3));


        //
        float thumb_d = THUMB_LENGTH;
        float index_d = INDEX_LENGTH;
        float middle_d = MIDDLE_LENGTH;

        float thumb_base_x = THUMB_B_X;
        float thumb_base_y = THUMB_B_Y;
        float thumb_base_z = THUMB_B_Z;

        float index_base_x = INDEX_B_X;
        float index_base_y = INDEX_B_Y;
        float index_base_z = INDEX_B_Z;

        float middle_base_x = MIDDLE_B_X;
        float middle_base_y = MIDDLE_B_Y;
        float middle_base_z = MIDDLE_B_Z;
        ...
        */
        ...
        //------------------------------------------------------------------------------
        // logger_finger_joints
        // !!!!!!
        //------------------------------------------------------------------------------

        //    finger_joint_logger(thumb_theta1, thumb_theta2, thumb_theta3, thumb_theta4, index_theta1, index_theta2, index_theta3, index_theta4, middle_theta1, middle_theta2, middle_theta3, middle_theta4);
        ...
        std::cout<<"----------------------------------------------------------------------------------"<<std::endl;
        std::cout<<"thumb: "<<thumb_theta1_<<","<<thumb_theta2_<<","<<thumb_theta3_<<","<<thumb_theta4_<<std::endl;

        std::cout<<"index: "<<index_theta1_<<","<<index_theta2_<<","<<index_theta3_<<","<<index_theta4_<<std::endl;

        std::cout<<"middle: "<<middle_theta1_<<","<<middle_theta2_<<","<<middle_theta3_<<","<<middle_theta4_<<std::endl;
