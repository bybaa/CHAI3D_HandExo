...
Eigen::Matrix4f finger4_pose;	//ring
...
void updateHaptics(void)
{
    ...
    while (simulationRunning)
    {
        ...
        OneEuroFilter angle18(frequency, mincutoff, beta, dcutoff);
        OneEuroFilter angle19(frequency, mincutoff, beta, dcutoff);
        OneEuroFilter angle20(frequency, mincutoff, beta, dcutoff);
        OneEuroFilter angle21(frequency, mincutoff, beta, dcutoff);
        OneEuroFilter angle22(frequency, mincutoff, beta, dcutoff);
        OneEuroFilter angle23(frequency, mincutoff, beta, dcutoff);
        ...
        std::cout << "raw angles : " <<angle[0]<<","<<angle[1]<<","<<angle[2]<<","<<angle[3]<<","<<angle[4]<<","<<angle[5]<<","<<
                                       ...
                                       angle[18]<<","<<angle[19]<<","<<angle[20]<<","<<angle[21]<<","<<angle[22]<<","<<angle[23]<<std::endl;
        */
        ...
        float filtered_angle18 = angle18.filter(angle[18], -1);
        float filtered_angle19 = angle19.filter(angle[19], -1);
        float filtered_angle20 = angle20.filter(angle[20], -1);
        float filtered_angle21 = angle21.filter(angle[21], -1);
        float filtered_angle22 = angle22.filter(angle[22], -1);
        float filtered_angle23 = angle23.filter(angle[23], -1);
        /*
        ...
        */
        ...
        finger4_pose = FingerMath::getF4TransMatrix(filtered_angle18, filtered_angle19, filtered_angle20, filtered_angle21, filtered_angle22, filtered_angle23);
        ...
        */
        ...
        //modify data
        ring_x = finger4_pose(0, 3) * 0.01;
        ring_y = finger4_pose(1, 3) * 0.01;
        ring_z = finger4_pose(2, 3) * 0.01;

        aux_ring_rot[0] = finger4_pose(0, 0);
        aux_ring_rot[1] = finger4_pose(0, 1);
        aux_ring_rot[2] = finger4_pose(0, 2);
        aux_ring_rot[3] = finger4_pose(1, 0);
        aux_ring_rot[4] = finger4_pose(1, 1);
        aux_ring_rot[5] = finger4_pose(1, 2);
        aux_ring_rot[6] = finger4_pose(2, 0);
        aux_ring_rot[7] = finger4_pose(2, 1);
        aux_ring_rot[8] = finger4_pose(2, 2);

        ring_position.set(ring_x, ring_y, ring_z);
        ring_rotation.set(aux_ring_rot[0], aux_ring_rot[1], aux_ring_rot[2],
            aux_ring_rot[3], aux_ring_rot[4], aux_ring_rot[5],
            aux_ring_rot[6], aux_ring_rot[7], aux_ring_rot[8]);
